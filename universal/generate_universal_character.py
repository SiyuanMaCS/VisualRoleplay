...
def generate_roles(round, dataset, result_path, dataset_root, num_init, num_per_round, num_demos,gpus): 
    ...
    sampling_params = SamplingParams(max_tokens=512, temperature=1, top_p=0.7)

    while len(new_roles_list) < num_per_round:
        batch_prompt = []
        for i in range(num_per_round):
            if round == 0:
                prompt = get_init_prompt_image_prompt(dataset, dataset_root)            
            else:

                for r in range(round):
                    roles_round=pd.read_csv(f"{result_path}/../{round}/roles.csv",index_col=0)
                    roles_list.append(roles_round)
                all_roles_df = pd.concat(roles_list)
                
                print(f"Round{round}: improving roles...")
                best_roles_df = all_roles_df.sort_values(by='jailbroken_asr',
                                                                        ascending=False).head(num_best_roles)
                
                
                if num_demos > len(best_roles_df):
                    num_demos = len(best_roles_df)
                sample_best_roles_df = best_roles_df.sample(num_demos)
                prompt = get_improve_prompt_image_prompt(dataset, dataset_root, sample_best_roles_df)
        ...
        for i, output in enumerate(outputs):
            ...
            if len(matches_square_brackets) == 1 and len(matches_curly_brackets) == 1 and len(
                ...
                appearance_description = matches_square_brackets[0]
                detailed_description = matches_curly_brackets[0]
                brief_description = matches_circle_brackets[0]

                new_row = pd.DataFrame([{"diffusion prompt": appearance_description,
                                         "detailed description": detailed_description,
                                         "brief description": brief_description,
                                         "round": round}])
                #print(new_row)
                new_roles_list.append(new_row)
                new_roles_df = pd.concat(new_roles_list, ignore_index=True)
                new_roles_df.to_csv(f"{result_path}/new_roles.csv")
    ...
    new_roles_df = pd.concat(new_roles_list, ignore_index=True)
    new_roles_df = new_roles_df.head(num_per_round)
...
def evaluate_relevant(batch_response_df, gpus):
    model_path = "/data/public_models/huggingface/mistralai/Mistral-7B-Instruct-v0.2"
    vllm_model = LLM(model_path,
                     tensor_parallel_size=gpus,
                     trust_remote_code=True,
...
def main():
    ...
    parser.add_argument('--image_root', type=str, default="./image_universal")
    ...
    parser.add_argument('--num_per_round', type=int, default=5)
    parser.add_argument('--num_best_roles', type=int, default=5)
    ...
    model_path = "/data/public_models/huggingface/mistralai/Mixtral-8x7B-Instruct-v0.1"
    ...
    result_path = f"{result_root}/{dataset}/train/{model_name}/{round}"

    image_path = f"{image_root}/{dataset}/train/{model_name}/{round}"

    os.makedirs(result_path, exist_ok=True)
    os.makedirs(image_path, exist_ok=True)


    if os.path.exists(f"{result_path}/new_roles.csv"):
        new_roles_df = pd.read_csv(f"{result_path}/new_roles.csv",index_col=0)
        new_roles_df = new_roles_df.reset_index(drop=True)
        print(f"{result_path}/new_roles.csv")
        print(new_roles_df)
        print("Skip, already saved.")
    else:
        roles_list = []
        
        new_roles_df = generate_roles(round,dataset,  result_path, dataset_root, num_init, num_per_round, num_demos,gpus)
        new_roles_df = new_roles_df.reset_index(drop=True)
        new_roles_df.to_csv(f"{result_path}/new_roles.csv")
    print(new_roles_df)
    ...
    if os.path.exists(f"{result_path}/batch_question.csv"):
        ...
        batch_question_df = dataset_df.sample(n=batch_size)
    ...
    batch_toxic_asr_sr = batch_evaluation_df.groupby('diffusion prompt')['jailbroken_toxic'].mean()
    
    batch_toxic_asr_dict = {'diffusion prompt': batch_toxic_asr_sr.index, 'jailbroken_toxic_asr': batch_toxic_asr_sr.values}
    batch_toxic_asr_df = pd.DataFrame(batch_toxic_asr_dict)
    new_roles_df = new_roles_df.merge(batch_toxic_asr_df, on='diffusion prompt')
    print(batch_toxic_asr_df)
    batch_relevant_asr_sr = batch_evaluation_df.groupby('diffusion prompt')['jailbroken_relevant'].mean()
    batch_relevant_asr_dict = {'diffusion prompt': batch_relevant_asr_sr.index, 'jailbroken_relevant_asr': batch_relevant_asr_sr.values}
    batch_relevant_asr_df = pd.DataFrame(batch_relevant_asr_dict)
    new_roles_df = new_roles_df.merge(batch_relevant_asr_df, on='diffusion prompt')


    batch_asr_sr = batch_evaluation_df.groupby('diffusion prompt')['jailbroken'].mean()
    batch_asr_dict = {'diffusion prompt': batch_asr_sr.index, 'jailbroken_asr': batch_asr_sr.values}
    batch_asr_df = pd.DataFrame(batch_asr_dict)
    new_roles_df = new_roles_df.merge(batch_asr_df, on='diffusion prompt')


    print(new_roles_df.loc[:, ['jailbroken_toxic_asr', 'jailbroken_relevant_asr', 'jailbroken_asr']])
